...
- Idle until CPU temp stabilizes (dT/dt < --stabilize-slope, default
  0.1 C/min, sustained over a --stabilize-window, default 120 s), then
...
def run_trial(args):
    ...
    print(f"  Min hold        : {args.min_hold_s:.0f}s (stability can't end a phase before this)")
    print(f"  Fast sampling   : {args.fast_poll_interval}s interval for the first "
          f"{args.fast_sample_s:.0f}s of step_up/step_down (resolves tau1)")
    print(f"  Ambient cadence : re-read every {args.ambient_check_interval:.1f}s "
          f"(DS18B20 conversion is the dominant per-row cost otherwise)")
    ...
    # get_throttle_status() still has to spawn vcgencmd -- there's no
    # sysfs equivalent for the throttle flags the way there is for temp/
    # clock. Throttle state doesn't need resampling every fast-sample
    # tick the way temperature does, so it's refreshed on a fixed cadence
    # (--throttle-check-interval, default 1.0s) instead of every row, and
    # the cached value is reused in between. This removes the last
    # subprocess spawn from the hot loop during fast sampling.
    last_throttle = {"raw_hex": "n/a", "under_voltage": False, "freq_capped": False,
                      "throttled": False, "soft_temp_limit": False}
    last_throttle_check_t = -1e9

    # The DS18B20 1-Wire ambient sensor was the real bottleneck limiting
    # sample rate, bigger than the vcgencmd subprocess overhead the sysfs
    # switch fixed. The Linux w1-therm driver triggers a fresh temperature
    # conversion on every single read of w1_slave and blocks until it
    # completes -- ~750ms at the default 12-bit resolution -- so reading
    # it every loop iteration put a ~0.75s floor under every row
    # regardless of --fast-poll-interval. Confirmed directly: after fixing
    # temp/clock via sysfs and caching throttle status, fast-sampling rows
    # were still landing at ~1-1.9s apart instead of the requested 0.2s,
    # which matches a ~0.75s DS18B20 conversion plus other overhead almost
    # exactly. Ambient room temperature has no business changing
    # meaningfully within a fraction of a second anyway, so it's now
    # cached and refreshed on a fixed cadence (--ambient-check-interval,
    # default 2.0s) instead of read every row.
    last_ambient = None
    last_ambient_check_t = -1e9
    ...
    try:
        with open(csv_path, "w", newline="") as f:
            ...
            while phase != "done":
                ...
                if t - last_throttle_check_t >= args.throttle_check_interval:
                    last_throttle = gc.get_throttle_status()
                    last_throttle_check_t = t
                throttle = last_throttle
                if args.ambient_sensor and t - last_ambient_check_t >= args.ambient_check_interval:
                    last_ambient = gc.read_ds18b20(args.ambient_sensor)
                    last_ambient_check_t = t
                ambient = last_ambient if args.ambient_sensor else None
                ...
                # Use a much finer sampling interval for the first
                # --fast-sample-s seconds after entering step_up or
                # step_down, then fall back to --poll-interval. This
                # exists because tau1 (the fast die/heatsink transient)
                # can be a few seconds on the geothermal condition -- at
                # the default 1s --poll-interval that's only 2-4 samples
                # across the entire fast branch, which is not enough to
                # resolve it well (see thermal_system_id.py's
                # fit_two_exponential). Idle phases don't need this: they
                # aren't fit for tau1/tau2, so they stay at the coarser
                # --poll-interval to avoid bloating the CSV for no
                # benefit.
                phase_elapsed_so_far = t - phase_start_t
                if phase in ("step_up", "step_down") and phase_elapsed_so_far < args.fast_sample_s:
                    effective_interval = args.fast_poll_interval
                else:
                    effective_interval = args.poll_interval

                if phase == "step_up":
                    if llm is not None:
                        # Bounded to effective_interval (fine near the
                        # step, coarser afterward) -- keeps step_up
                        # logging just as densely as idle by default, and
                        # much more densely right after the step.
                        run_inference_burst(llm, STRESS_PROMPT, effective_interval,
                                            max_tokens_per_call=args.max_tokens)
                    elif not args.simulate:
                        busy_stress(seconds=min(2.0, effective_interval))
                    else:
                        time.sleep(min(0.05, effective_interval))
                else:
...
def _advance_phase(phase, stable, phase_elapsed, args):
    ...
    min_hold_s is a hard floor applied only to step_up and step_down: those
    are the phases that get fit for tau1/tau2 in thermal_system_id.py, so
    they're the ones that need to run long enough for the slow branch to
    show itself before "stable" is allowed to end them. idle_pre and
    idle_post are not fit the same way and never showed this truncation
    problem, so they keep the original stable-or-timeout behavior
    unconditionally -- otherwise a --min-hold-s tuned for step_down's slow
    tail (e.g. 1000s) would also force every idle phase to sit for 1000s
    before it's allowed to start, even once it's genuinely at rest. The
    slope check only looks at the trailing --stabilize-window seconds --
    for a slow branch (tau2 in the hundreds of seconds, as seen on the
    geothermal condition), that window can look flat well before the
    temperature has actually finished moving, since a slow exponential's
    rate of change keeps shrinking throughout the tail. Without this floor,
    step_up/step_down can end with the slow branch barely captured, leaving
    thermal_system_id.py's two-exponential fit too little data to resolve
    tau2 -- it either fails to converge or lands on a near-degenerate
    tau1≈tau2 fit instead (see analyze_step's convergence/degeneracy
    warnings). The max-wait ceiling still applies unconditionally on top of
    this wherever it's used, so a bad sensor or drafty room still can't
    hang the trial forever even if min_hold_s is set larger than intended.
    """
    if phase == "idle_pre":
...
def _fit_and_report(csv_path, args, step_up_time, step_down_time):
    ...
    if len(results_df) == 0:
        ...
        for _, row in results_df.iterrows():
            ...
            if _is_nan(row["tau1_two_exp_s"]) or _is_nan(row["tau2_two_exp_s"]):
                print(f"    two-exponential fit did not converge on this window -- see the "
                      f"warning printed during fitting above. No tau/R_th to report for this step.")
            else:
                ...
                if "R_thermal_c_per_w" in row and not _is_nan(row["R_thermal_c_per_w"]):
                    print(f"    R_th (R1+R2)   : {row['R_thermal_c_per_w']:.3f} C/W")
                if "R1_two_exp_c_per_w" in row and not _is_nan(row.get("R1_two_exp_c_per_w")):
                    print(f"    R1 / C1        : {row['R1_two_exp_c_per_w']:.3f} C/W / {row['C1_two_exp_j_per_c']:.1f} J/C")
                    print(f"    R2 / C2        : {row['R2_two_exp_c_per_w']:.3f} C/W / {row['C2_two_exp_j_per_c']:.1f} J/C")
...
def main():
    ...
    ap.add_argument("--stabilize-slope", type=float, default=0.1,
                     help="Stabilization threshold in C/min (default: 0.1)")
    ...
    ap.add_argument("--min-hold-s", type=float, default=0.0,
                     help="Hard floor in seconds applied to step_up and step_down only "
                          ...
                          "slope check can look flat well before a slow branch (large tau2) has "
                          "actually finished moving, since a slow exponential's rate of change "
                          "keeps shrinking throughout its tail -- this was observed on a "
                          "geothermal step_down phase that ended at ~140s and left "
    ...
    ap.add_argument("--fast-sample-s", type=float, default=30.0,
                     help="Duration in seconds, measured from the moment step_up or step_down "
                          "begins, during which samples are taken at --fast-poll-interval instead "
                          "of --poll-interval (default: 30). This targets the fast branch (tau1), "
                          "which on the geothermal condition can be just a few seconds -- the "
                          "default 1s --poll-interval only gives 2-4 samples across a transient "
                          "that short, which isn't enough for fit_two_exponential to resolve tau1 "
                          "well. Set to 0 to disable and always use --poll-interval.")
    ap.add_argument("--fast-poll-interval", type=float, default=0.2,
                     help="Sampling interval in seconds used during the first --fast-sample-s "
                          "seconds of step_up/step_down (default: 0.2 = 5 samples/sec). "
                          "cpu_temp_c/cpu_clock_mhz are read from sysfs (fast, no subprocess "
                          "spawn), so this is limited mainly by --throttle-check-interval's "
                          "cached vcgencmd call and, during step_up, by inference-burst timing -- "
                          "very small values may still not be fully achievable and the loop will "
                          "simply run as fast as it can.")
    ap.add_argument("--throttle-check-interval", type=float, default=1.0,
                     help="Minimum seconds between get_throttle_status() calls (default: 1.0). "
                          "That call still has to spawn vcgencmd (no sysfs equivalent, unlike "
                          "temp/clock), so it's checked on this fixed cadence and the last value "
                          "reused in between rather than being called every row -- this matters "
                          "most during fast sampling, where a subprocess spawn every 0.2s would "
                          "otherwise eat most of that budget. Lower it if you specifically need "
                          "tighter throttle-flag resolution; it has no effect on the CSV's "
                          "cpu_temp_c/cpu_clock_mhz columns, which are read from sysfs every row "
                          "regardless.")
    ap.add_argument("--ambient-check-interval", type=float, default=2.0,
                     help="Minimum seconds between DS18B20 ambient-sensor reads (default: 2.0). "
                          "The Linux w1-therm driver triggers a fresh conversion on every read of "
                          "w1_slave and blocks until it completes (~750ms at default 12-bit "
                          "resolution), so reading it every row was putting a ~0.75s floor under "
                          "every sample regardless of --fast-poll-interval -- this was the "
                          "dominant remaining bottleneck after switching cpu_temp_c/cpu_clock_mhz "
                          "to sysfs. Ambient room temperature doesn't change meaningfully within "
                          "a fraction of a second, so it's now cached and refreshed on this fixed "
                          "cadence instead. Has no effect on cpu_temp_c, which is unaffected and "
