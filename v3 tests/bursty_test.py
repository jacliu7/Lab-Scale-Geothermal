...
def load_calibration(tau_csv_path, condition, row_index=0):
    ...
    given cooling condition and build a predictor function from it.
    ...
    calib = {"model": model, "label": row["label"], "source_row": row_index}

    def _single_exp_fallback(reason):
        print(f"  Note: {reason} Falling back to this row's single-exponential fit "
              f"(tau_nonlinear_s / R_thermal_c_per_w, computed for every row regardless "
              f"of which model thermal_system_id.py chose to report).")
        for col in ["tau_nonlinear_s", "R_thermal_c_per_w"]:
            if col not in row or pd.isna(row[col]):
                raise ValueError(
                    f"Two-exponential fit was degenerate and no usable single-exponential "
                    f"fallback is available in this row either (missing '{col}'). Re-run the "
                    f"step test for a cleaner calibration, ideally with a longer hold."
                )
        return {
            "model": "single_exponential", "label": row["label"], "source_row": row_index,
            "tau": float(row["tau_nonlinear_s"]), "R_th": float(row["R_thermal_c_per_w"]),
        }

    if model == "two_exponential":
        ...
        # Near-degenerate check: when tau1 and tau2 are close together, the
        # two branches are nearly collinear and A1/A2 (hence R1/R2) become
        # numerically unstable -- they can individually blow up to
        # unphysical values while still summing to a perfectly reasonable
        # total delta_T and a good R^2 on the original step. That's fine
        # for reporting tau on the one step it was fit to, but forward-
        # simulating an arbitrary Q(t) with runaway R1/R2 produces garbage.
        # Also sanity-check R1+R2 against the always-computed single-node
        # R_thermal_c_per_w for this same row -- they should be in the same
        # ballpark if the two-branch decomposition is physically meaningful.
        ...
        if tau_ratio < 1.3 or r_ratio_bad:
            calib = _single_exp_fallback(
                f"Two-exponential branches for '{row['label']}' are near-degenerate "
                f"(tau1={tau1:.1f}s, tau2={tau2:.1f}s, R1={R1:.2f}, R2={R2:.2f} C/W, "
                f"vs. single-node R_th={r_single}) -- R1/R2 aren't trustworthy for simulation."
            )
        else:
            calib.update({"tau1": tau1, "tau2": tau2, "R1": R1, "R2": R2})
    else:
        for col in ["tau_nonlinear_s", "R_thermal_c_per_w"]:
            if col not in row or pd.isna(row[col]):
                raise ValueError(
        ...
        calib.update({
            ...
            "R_th": float(row["R_thermal_c_per_w"]),
        })
...
class RCPredictor:
    """
    Drives the calibrated single- or two-exponential thermal model forward
    through a measured Q(t) sequence, one sample at a time, using the
    exact analytic update for a first-order (or Foster two-branch) system
    with piecewise-constant input over each sample interval. This is not
    a new fit -- every parameter comes from load_calibration() -- it's
    just forward simulation for comparison against the measured curve.
    """

    def __init__(self, calib):
        self.calib = calib
        self.model = calib["model"]
    ...
    def step(self, dt_s, Q_w):
        ...
        if self.model == "two_exponential":
            ...
            tau, R_th = self.calib["tau"], self.calib["R_th"]
            target = Q_w * R_th
            self.x = target + (self.x - target) * np.exp(-dt_s / tau)
        return self.predicted_delta_t()
...
def run_trial(args):
    ...
    print(f"  Calibration     : {calib['model']} from '{calib['label']}' "
          f"(row {args.calibration_row} of {args.tau_source})")
    if calib["model"] == "two_exponential":
